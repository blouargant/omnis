...
| `/spawn <name> [squad] [task]` | Start a **fresh** session (empty context — *not* a copy of this conversation) rooted in this session's working directory. The first argument is the name, the second the squad; anything after is an optional initial **task** the new session starts working on **in the background** — you're notified when it replies. **Omit the squad** to let the Omnis router pick the best-suited squad for the task. Useful for handing off a separate, parallel job. A squad **leader** can do the same on its own via the `spawn_session` tool (disable it by removing the `spawn` tool from the leader in Settings → Agent). |
