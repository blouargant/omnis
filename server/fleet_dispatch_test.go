...
// filed under its own collection and rooted at its cwd; a project on an engine
// with no squad yet (claude), or an unknown project name, is rejected (ok=false)
// so the caller skips it rather than mis-dispatching.
func TestFleetDriverOptions(t *testing.T) {
	...
	if _, ok := fleetDriverOptions("Service B", "alice"); ok {
		t.Fatal("expected ok=false for a claude-engine project (no squad yet)")
	}
