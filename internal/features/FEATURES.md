...
- **Fleets in the sidebar** — group your projects into named fleets, each with its own colour and default engine, and start a coordinated chat on one from its Coordinate button. The Fleets area is always there, with a one-click "Create a fleet" when you have none yet.
- **Create a project, don't repurpose a folder** — a fleet's "New project…" makes a project of its own, asking for what one actually needs: its workspace directory (checked as you type, with path completion), its engine, and which other projects it depends on. Existing collections can still be turned into projects, and any project can be moved between fleets.
