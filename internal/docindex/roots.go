...
// Roots returns the directories whose markdown files make up yoke's
// documentation corpus. The result is deduplicated and contains only
// directories that actually exist.
//
// When YOKE_DOCS_DIRS is set (colon-separated) it replaces the auto-discovered
// set wholesale. Otherwise the candidates are, in priority order:
//
//   - <webDir>/docs        (the web UI user docs; webDir = YOKE_WEB_DIR or "web")
//   - /usr/share/yoke/web/docs   (packaged web UI docs)
//   - ./docs               (developer docs in a source checkout)
//   - /usr/share/doc/yoke/docs   (packaged developer docs)
//   - <exeDir>/docs        (developer docs next to the binary)
func Roots() []string {
	if v := strings.TrimSpace(os.Getenv("YOKE_DOCS_DIRS")); v != "" {
		return existingDirs(strings.Split(v, ":"))
	}

	webDir := strings.TrimSpace(os.Getenv("YOKE_WEB_DIR"))
	if webDir == "" {
		webDir = "web"
	}
	...
	candidates := []string{
		...
		"/usr/share/yoke/web/docs",
		"docs",
		"/usr/share/doc/yoke/docs",
	}
