...
| `OMNIS_SERVER_ADDR` | (`addr`) | Listen address inside the container. Rendered from the platform's `OMNIS_LISTEN_ADDR` in the supervisord template — never hard-coded. Use `127.0.0.1:8080` when the gateway proxies from a sidecar inside the same pod; use a non-loopback bind (e.g. `0.0.0.0:8080`) when the gateway reaches the pod over the cluster network — omnis is otherwise unreachable. |
...
**Charset note:** the supervisord template is rendered with `envsubst`, and
supervisord itself then applies Python `%(...)s` interpolation to every value
in the file — a `%` or `"` in `OMNIS_SERVER_TOKEN` or the LiteLLM key would
make the rendered program unparseable. Constrain both to `[A-Za-z0-9._-]`
(hex or base64url secrets satisfy this already), or have the container
entrypoint escape `%` as `%%` before running `envsubst`.
...
| `packaging/container/networkpolicy.example.yaml` | Example policy allowing the omnis port only from the gateway. Applies only to a non-loopback bind — with `OMNIS_SERVER_ADDR` bound to `127.0.0.1`, nothing outside the pod can reach the port anyway and this policy is moot. |
...
The identity header check still applies in this mode, including on the
terminal WebSocket (`GET /api/terminal/ws`): that route sits outside the
token-checking group (browsers cannot set headers on a WS handshake), but it
carries the same identity middleware directly, so a request with no login or
the wrong one is refused before any upgrade is attempted — even when there is
no token to check first.
...
- **The A2A listener, if enabled.** Setting `a2a_enabled: true` opens a second
  listener (`server/a2a_server.go`) gated only by its own optional bearer
  token (`authOK`) — it has **no** identity check at all, unlike `/api/*`. The
  container `server.yaml` ships with `a2a_enabled: false`; **keep it off in
  this deployment** until a milestone-2 hub gives cross-container A2A calls an
