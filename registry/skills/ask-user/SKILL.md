...
Call the `AskUserQuestion` tool. It **blocks** until the user responds and returns the
...
Before calling `AskUserQuestion`, run these checks:
...
| Question scope | **One `AskUserQuestion` call per turn.** Never stack multiple calls sequentially before reading results. |
...
After calling `AskUserQuestion`, the tool blocks — the LLM turn does not end until the
...
**before** calling `AskUserQuestion` for the blocked branch, and state what you are
doing in parallel.
