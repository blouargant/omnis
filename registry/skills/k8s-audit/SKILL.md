...
### Phase 3b — RBAC extraction recipe (deterministic)

For an RBAC rule (which ClusterRoles/Roles grant a permission on a resource),
follow this mechanically instead of shortlisting roles by name.
...
8. **RBAC recipe is mandatory** (Phase 3b). For an RBAC rule, filter the complete
   role set by resource+verb (wildcards match) and flag aggregation ClusterRoles
   by their own `.rules` — never shortlist by name.
