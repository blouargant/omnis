...
#   Linux (Debian/Ubuntu): sudo apt install texlive-latex-recommended texlive-xetex
#   Linux (Fedora/RHEL):   sudo dnf install texlive-scheme-basic texlive-xetex
#   macOS (Homebrew):      brew install --cask basictex      # Smaller (~100MB)
#                          brew install --cask mactex-no-gui # Full (~4GB)
# macOS only, after install: eval "$(/usr/libexec/path_helper)" or new terminal
...
export PATH="/Library/TeX/texbin:$PATH"   # macOS only
...
# Open and print to PDF (Ctrl+P / Cmd+P > Save as PDF)
...
# Open for review
xdg-open PSI-document.docx   # Linux
open PSI-document.docx       # macOS
```
...
# macOS - smaller option (~100MB)
brew install --cask basictex

# macOS - full option (~4GB)
brew install --cask mactex-no-gui

# macOS only - after install, update PATH
